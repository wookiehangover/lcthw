...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
  ...
  if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
    ...
    // recursive process the devpkg list
    ...
    // this indicated nothing needs to be done
    return 0;
    ...
    if(info.scheme) {
      ...
      check(rc == 0, "Faile to curl source: %s", url);
    }
  ...
  // indicated that an install needs to actually run
  return 1;
error:
  return -1;
}
    ...
    const char *make_opts, const char *install_opts)
{
  ...
  check(access(BUILD_DIR, X_OK | R_OK | W_OK) == 0,
      "Build director doesn't exist: %s", BUILD_DIR);
  ...
  rc = DB_update(url);
  check(rc == 0, "Failed to add this packeage to the database.");
    ...
    const char *make_opts, const char *install_opts)
{
  ...
  if(rc == 1) {
    log_info("Packacge %s already installed", url);
    return 0;
  }
  ...
  if(rc == 1) {
    ...
    // no install needed
    log_info("Deoends successfully installed: %s", url);
  } else {
    // had an error
